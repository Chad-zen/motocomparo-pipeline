-- ============================================================================
-- mcpipe core schema  (PostgreSQL 16)
--
...
CREATE INDEX feed_run_merchant_idx ON feed_run (merchant_id, started_at DESC);
...
CREATE TABLE raw_offer (
    ...
    product_id      bigint,                 -- FK added after `product` exists
    link_method     text,                  -- 'gtin_exact' | 'item_group' | 'base_sku' | 'fuzzy' | 'operator'
    link_confidence numeric(4, 3),
    UNIQUE (merchant_id, merchant_sku)
);
CREATE INDEX raw_offer_gtin_idx    ON raw_offer (raw_gtin) WHERE raw_gtin IS NOT NULL;
CREATE INDEX raw_offer_status_idx  ON raw_offer (linked_status);
CREATE INDEX raw_offer_product_idx ON raw_offer (product_id) WHERE linked_status = 'linked';
CREATE INDEX raw_offer_title_trgm  ON raw_offer USING gin (raw_title gin_trgm_ops);

-- one computed signature per raw_offer (successor of v1's wp_mc_psig)
CREATE TABLE offer_signature (
...
CREATE INDEX offer_signature_identity_idx ON offer_signature (identity_hash);
CREATE INDEX offer_signature_basesku_idx  ON offer_signature (base_sku) WHERE base_sku IS NOT NULL;
...
CREATE INDEX product_lookup_idx ON product (brand_code, category_id, model_core_ref)
...
CREATE TABLE offer_variant_link (
...
ALTER TABLE raw_offer
    ADD CONSTRAINT raw_offer_product_fk FOREIGN KEY (product_id) REFERENCES product(id);
...
CREATE INDEX match_review_pending_idx ON match_review_queue (status, priority);
